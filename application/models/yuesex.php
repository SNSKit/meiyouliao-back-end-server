...
class Yuesex extends CI_Model{
  ...
  public function allTopics(){
    ...
    $sql="select count(sex_id) as totalTopic from $this->picTable";
    ...
    if($page>$totalPage){
      ...
      $sql="select 
              t.sex_id,t.content,t.pic_name,t.ext,t.path,t.width,t.height,t.num_praise,t.num_trample,t.num_remark,t.create_time,
	  ...
	  if(!$arrTopics){
		...
		foreach($arrTopics as $key=>&$item){
		  $item['url']=$this->config->base_url().'static/sex/'.$item['path'].'/'.$item['pic_name'].$item['ext'];
		  if($item['avatar']==0){
  ...
  public function getTopic(){
    ...
    // get one topic..
    $sql="select 
            t.sex_id,t.user_id,t.content,t.pic_name,t.ext,t.path,t.width,t.height,t.num_praise,t.num_trample,t.num_remark,t.create_time,
            u.nickname,u.avatar
          from 
            $this->picTable as t
          left join
	...
	if($topic){
	  ...
	  $topic[0]['url']=$this->config->base_url().'static/sex/'.$topic[0]['path'].'/'.$topic[0]['pic_name'].$topic[0]['ext'];
      if($topic[0]['avatar']==0){
	    $avatar=$this->config->base_url().'static/avatar/df.png';
	  }else{
	  ...
	  if($this->upload->do_upload('pic')){
        ...
        return array(
          ...
          'topic'=>array(
			...
			'height'=>$height,
			'ext'=>$ext,
			'url'=>$this->config->base_url().'static/sex/'.$dirName.'/'.$picName.$ext,
			'num_praise'=>0,
			'num_trample'=>0,
			'num_remark'=>0,
  ...
  public function delTopic(){
    ...
    // auth user and topic..
    $sql="select user_id from $this->picTable where sex_id={$sex_id}";
    ...
    // start mysql transaction ...
  	$this->db->trans_begin();
  	// delete topic
  	$sql="delete from $this->picTable where sex_id={$sex_id}";
  	$this->db->query($sql);
    // delete remarks
  	$sql="delete from $this->picRemarkTable where sex_id={$sex_id}";
    $this->db->query($sql);
    // delete topic action record
    $sql="delete from $this->picActionTable where sex_id={$sex_id}";
    $this->db->query($sql);
    // delete personal topic record ..
    $sql="delete from $this->userPicTable where sex_id={$sex_id}";
    $this->db->query($sql);
    if($this->db->trans_status()===false){
      $this->db->trans_rollback();
      return array(
  ...
  public function addRemark(){
    ...
    $create_time=time();
    $sql="insert into 
            $this->picRemarkTable(sex_id,user_id,content,create_time) 
            values({$sex_id},{$user_id},'{$content}',${create_time})";
    $this->db->query($sql);
    $remark_id=$this->db->insert_id();
    // update remark number..
    $sql="update $this->picTable set num_remark=num_remark+1 where sex_id={$sex_id}";
    $this->db->query($sql);
    if($this->db->trans_status()===false){
      $this->db->trans_rollback();
      return array(
  ...
  public function actionTopic(){
    ...
    //}
    // check if exists
    //$sql="select user_id from $this->picActionTable where sex_id={$sex_id}";
    ...
    $this->db->trans_begin();
    //$sql="insert into 
            //$this->picActionTable(sex_id,user_id,action,create_time)
            //values({$sex_id},{$user_id},{$action},{$create_time})";
    //$this->db->query($sql);
    if($action==8){
      $sql="update $this->picTable set num_praise=num_praise+1 where sex_id={$sex_id}";
      $this->db->query($sql);
    }else if($action==4){
      $sql="update $this->picTable set num_trample=num_trample+1 where sex_id={$sex_id}";
      $this->db->query($sql);
    }
    if($this->db->trans_status()===false){
      $this->db->trans_rollback();
